      ...
      <div className="flex items-center justify-between mb-2 px-1">
        ...
        <div className="flex gap-1">
          <button
            ...
            className="p-1 text-slate-500 hover:text-slate-800 hover:bg-slate-200 rounded-lg transition-all"
          >
        ...
        {cells.map((cell, i) => {
          ...
          return (
            <button
              ...
              className={`w-full text-center text-xs py-1 font-semibold rounded-lg cursor-pointer transition-all ${
                isToday
...
function Sidebar({
  ...
  const [editCalColor, setEditCalColor] =
    useState<UserCalendar["color"]>("blue");
    ...
    <div className="w-full h-full bg-slate-50 flex flex-col py-6 shrink-0 overflow-y-auto">
          ...
          >
            ...
            >
              ...
              <circle
                ...
                cy="16"
                r="11.5"
                fill="url(#aura-logo-gradient)"
                opacity="0.28"
                filter="url(#aura-logo-glow)"
              />
              <path
      ...
      <div className="mb-3 px-4">
        <button
          ...
          className="flex h-10 w-full items-center justify-start gap-2 rounded-full bg-slate-100 px-4 text-xs font-extrabold text-slate-700 transition-all hover:bg-slate-200 hover:text-slate-900 active:scale-[0.98]"
        >
        ...
        <div className="flex items-center justify-between mb-2 px-2">
          ...
          <div className="flex items-center gap-0.5">
            <button
              ...
              className="calendar-action-button p-1 text-slate-400 hover:text-blue-600 hover:bg-slate-200/60 rounded-lg transition-all"
            >
          ...
          {calendars.map((cal) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div
                    className={`w-3.5 h-3.5 rounded-lg border-2 flex items-center justify-center shrink-0 transition-all ${
                      isVisible
                  ...
                  <span className="text-sm font-semibold text-slate-700 text-left truncate">
                    {cal.name}
                  </span>
                </button>
                ...
                <div
                  className="relative"
                  ref={calendarMenuId === cal.id ? calendarMenuRef : undefined}
                >
                  <button
                    type="button"
                    onClick={() =>
                      setCalendarMenuId(
                        calendarMenuId === cal.id ? null : cal.id,
                      )
                    }
                  ...
                  {calendarMenuId === cal.id && (
                    <div className="absolute right-0 top-full mt-1 z-30 w-48 bg-white border border-slate-200 rounded-xl p-1.5 shadow-xl animate-in fade-in zoom-in-95 duration-150">
                      <button
                        type="button"
                        onClick={() => {
                          onExportCalendar(cal.id);
                          setCalendarMenuId(null);
                        }}
                        className="w-full flex items-center gap-2 text-left px-3 py-2 rounded-lg text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2 text-left px-3 py-2 rounded-lg text-xs font-bold text-rose-600 hover:bg-rose-50 transition-colors"
                      >
                        ...
                        <div className="flex flex-nowrap gap-2">
                          {CALENDAR_COLORS.map((color) => (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                onUpdateCalendar(cal.id, cal.name, color)
                              }
                              aria-label={`Use ${color}`}
                              className={`h-5 w-5 shrink-0 rounded-full ${COLOR_DOT[color]} flex items-center justify-center transition-transform hover:scale-110`}
                            >
                              {cal.color === color && (
                                <Check
                                  size={12}
        ...
        {settingsOpen && (
          <div className="absolute bottom-full left-4 right-4 mb-1 z-50 bg-white border border-slate-200 rounded-2xl p-1.5 shadow-xl animate-in fade-in slide-in-from-bottom-2 duration-150">
            ...
            >
              ...
              <span
                className={`w-8 h-4 rounded-full p-0.5 transition-colors ${isDarkMode ? "bg-blue-600" : "bg-slate-200"}`}
              >
                <span
                  className={`block w-3 h-3 rounded-full bg-white transition-transform ${isDarkMode ? "translate-x-4" : ""}`}
                />
              </span>
            </button>
            <button
