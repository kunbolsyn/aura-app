...
import { createIcsCalendar } from "./lib/ics";
import type { UserEvent as CalendarEvent } from "./types";
...
function App() {
  const [isDarkMode, setIsDarkMode] = useState(() =>
    load("aura-dark-mode", false),
  );
  const [user, setUser] = useState<User | null>(null);
  const [authReady, setAuthReady] = useState(false);
  const [workspaceReady, setWorkspaceReady] = useState(false);
  const [workspaceLoadedFor, setWorkspaceLoadedFor] = useState<string | null>(
    null,
  );
  ...
  const [icsImportFiles, setIcsImportFiles] = useState<File[] | null>(null);
  ...
  useEffect(() => {
    ...
    void restoreSession();
    const { data: listener } = configuredClient.auth.onAuthStateChange(
      (_event, session) => {
        if (!mounted) return;
  ...
  useEffect(() => {
    ...
    void loadWorkspace()
      .then((workspace) => {
        if (!mounted) return;
        const nextLists = workspace.lists.length
          ? workspace.lists
          : DEFAULT_LISTS;
        const nextCalendars = workspace.calendars.length
          ? workspace.calendars
          : DEFAULT_CALENDARS;
  ...
  function openIcsImport(files: File[] = []) {
    setIcsImportFiles(files);
  }

  function addImportedEvents(importedEvents: CalendarEvent[]) {
    setEvents((previous) => [...previous, ...importedEvents]);
  }

  function exportCalendar(calendarId: string) {
    const calendar = calendars.find((item) => item.id === calendarId);
    if (!calendar) return;
    const content = createIcsCalendar(
      calendar.name,
    ...
    link.download = `${calendar.name.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "") || "calendar"}.ics`;
    link.click();
    ...
    <div className="aura-app flex flex-col lg:flex-row h-screen bg-slate-50 text-slate-800 overflow-hidden font-sans">
      ...
      >
        <Sidebar
          ...
          onCreateEvent={() => setCreateEventRequest((count) => count + 1)}
          onOpenIcsImport={() => openIcsImport()}
      ...
      {icsImportFiles && (
        <IcsImportDialog
          calendars={calendars}
          initialFiles={icsImportFiles}
          onAddEvents={addImportedEvents}
          onClose={() => setIcsImportFiles(null)}
        />
      )}
    </div>
  );
